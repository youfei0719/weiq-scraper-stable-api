...
def _normalize_post_item(item: dict[str, Any]) -> dict[str, Any] | None:
...
def extract_post_trend_from_payloads(payloads: list[Any], limit: int = 20) -> list[dict[str, Any]]:
    """Extract recent-post data from WEIQ's structured page responses."""
    ...
    def walk(value: Any) -> None:
        if isinstance(value, dict):
            ...
            posts = [post for post in (_normalize_post_item(item) for item in value if isinstance(item, dict)) if post]
            if posts:
    ...
    seen: set[str] = set()
    best = max(candidates, key=lambda items: (len(items), sum(1 for item in items if item["content"])))
    for item in best:
        if item["source_post_id"] not in seen:
...
def extract_post_trend_from_echarts_options(options: list[Any], limit: int = 20) -> list[dict[str, Any]]:
    """Use chart labels and reads when WEIQ omits the structured post response."""
    def walk(value: Any) -> list[dict[str, Any]]:
        if isinstance(value, dict):
            x_axis, series = value.get("xAxis"), value.get("series")
            axes = x_axis if isinstance(x_axis, list) else [x_axis]
            labels = next((axis.get("data") for axis in axes if isinstance(axis, dict) and isinstance(axis.get("data"), list)), [])
            rows = series if isinstance(series, list) else [series]
            for row in sorted((item for item in rows if isinstance(item, dict)), key=lambda item: int("阅读" in str(item.get("name") or "") or "read" in str(item.get("name") or "").lower()), reverse=True):
                data = row.get("data")
                if not isinstance(data, list) or not labels:
                    continue
                posts = []
                for label, raw_reads in zip(labels, data):
                    reads = _parse_metric_number(raw_reads.get("value") if isinstance(raw_reads, dict) else raw_reads)
                    published_at = _normalize_post_text(label, 64)
                    if reads > 0 and published_at:
                        fingerprint = f"{published_at}||{int(reads)}"
                        posts.append({"source_post_id": f"hash:{hashlib.sha256(fingerprint.encode('utf-8')).hexdigest()[:24]}", "published_at": published_at, "reads": int(round(reads)), "content": "", "cover_url": None, "post_url": None})
                if posts:
